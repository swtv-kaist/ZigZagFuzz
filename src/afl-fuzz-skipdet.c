...
u8 skip_deterministic_stage(afl_state_t *afl, u8 *orig_buf, u8 *out_buf,
                            u32 len, u64 before_det_time, u8 *argv_buf,
                            u32 argv_len) {
  ...
  if (common_fuzz_stuff(afl, orig_buf, len, argv_buf, argv_len)) { return 0; }
  ...
  if (MINIMAL_BLOCK_SIZE * 8 < len) {
    ...
    while (pos < len - 1) {
      ...
      while (cur_block_size < max_block_size) {
        ...
        if (common_fuzz_stuff(afl, out_buf, len, argv_buf, argv_len)) return 0;
