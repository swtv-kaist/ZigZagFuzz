...
u8 fuzz_one_original(afl_state_t *afl) {
  u32 len, temp_len, argv_len;
  u32 j;
  u32 i;
  u8 *in_buf, *out_buf, *orig_in, *ex_tmp, *eff_map = 0, *argv_buf;
  u64 havoc_queued = 0, orig_hit_cnt, new_hit_cnt = 0, prev_cksum, _prev_cksum;
  u32 splice_cycle = 0, perf_score = 100, orig_perf, eff_cnt = 1, argv_id;
  u32 total_run = 0;
  u64 begin_hit_cnt = 0;
  ...
  argv_buf = afl->queue_cur->argv;
  argv_len = afl->queue_cur->argv_len;
  ...
  if (likely(!afl->old_seed_selection))
    ...
    orig_perf = calculate_score(afl, afl->queue_cur);
    afl->queue_cur->perf_score = perf_score = orig_perf;
  }
...
#define FLIP_BIT(_ar, _b)                     \
  ...
  if (common_fuzz_stuff(afl, out_buf, len, argv_buf, argv_len)) {
    goto abandon_entry;
  }
...
skip_interest:
  ...
  for (i = 0; i <= (u32)len; ++i) {
    ...
    for (j = 0; j < afl->extras_cnt; ++j) {
      ...
      if (common_fuzz_stuff(afl, ex_tmp, len + afl->extras[j].len, argv_buf,
                            argv_len)) {
        goto abandon_entry;
      }
...
skip_user_extras:
  ...
  for (i = 0; i <= (u32)len; ++i) {
    ...
    for (j = 0; j < afl->a_extras_cnt; ++j) {
      ...
      if (common_fuzz_stuff(afl, ex_tmp, len + afl->a_extras[j].len, argv_buf,
                            argv_len)) {
        goto abandon_entry;
      }
    ...
    if (el->afl_custom_fuzz) {
      ...
      if (afl->stage_max) {
        for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max;
          ...
          if (mutated_size > 0) {
            if (common_fuzz_stuff(afl, mutated_buf, (u32)mutated_size, argv_buf,
                                  argv_len)) {
              goto abandon_entry;
            }
...
#define MUTATE_ASCII_DICT 64
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    for (i = 0; i < use_stacking; ++i) {
      ...
      switch ((r = rand_below(afl, r_max))) {
        case 0 ... 3: {
          ...
          change_loc = rand_below(afl, temp_len << 3);
          FLIP_BIT(out_buf, change_loc);
...
#ifdef MUTATION_COUNT
          if ((change_loc >> 3) < ARGV_MAX_SIZE) {
            afl->fsrv.argv_op_execs++;
          } else {
            afl->fsrv.file_op_execs++;
          }
#endif
        ...
        case 4 ... 7: {
          ...
          change_loc = rand_below(afl, temp_len);
          out_buf[change_loc] =
        ...
        case 8 ... 9: {
          ...
          if (temp_len < 2) { break; }
          change_loc = rand_below(afl, temp_len - 1);
          *(u16 *)(out_buf + change_loc) =
          ...
          if (change_loc >= ARGV_MAX_SIZE) {
            afl->fsrv.file_op_execs++;
          } else if (change_loc == ARGV_MAX_SIZE - 1) {
            afl->fsrv.both_op_execs++;
          } else {
            afl->fsrv.argv_op_execs++;
          }
#endif
        ...
        case 12 ... 13: {
          ...
          *(u32 *)(out_buf + change_loc) =
          ...
          if (change_loc >= ARGV_MAX_SIZE) {
            afl->fsrv.file_op_execs++;
          } else if (change_loc < ARGV_MAX_SIZE - 3) {
            afl->fsrv.argv_op_execs++;
          } else {
            afl->fsrv.both_op_execs++;
          }
        ...
        case 16 ... 19: {
          ...
          change_loc = rand_below(afl, temp_len);
          out_buf[change_loc] -= 1 + rand_below(afl, ARITH_MAX);
...
#endif
#ifdef MUTATION_COUNT
          if (pos >= ARGV_MAX_SIZE) {
            afl->fsrv.file_op_execs++;
          } else if (pos == ARGV_MAX_SIZE - 1) {
            afl->fsrv.both_op_execs++;
          } else {
            afl->fsrv.argv_op_execs++;
          }
        ...
        case 40 ... 43: {
          ...
          change_loc = rand_below(afl, temp_len);
          out_buf[change_loc] ^= 1 + rand_below(afl, 255);
...
#endif
#ifdef MUTATION_COUNT
            if (copy_to >= ARGV_MAX_SIZE) {
              afl->fsrv.file_op_execs++;
            } else if (copy_to + copy_len >= ARGV_MAX_SIZE) {
              afl->fsrv.both_op_execs++;
            } else {
              afl->fsrv.argv_op_execs++;
            }
...
#endif
#ifdef MUTATION_COUNT
          if ((switch_to >= ARGV_MAX_SIZE) && (switch_from >= ARGV_MAX_SIZE)) {
            afl->fsrv.file_op_execs++;
          } else if ((switch_to + switch_len < ARGV_MAX_SIZE) &&
                     (switch_from + switch_len < ARGV_MAX_SIZE)) {
            afl->fsrv.argv_op_execs++;
          } else {
            afl->fsrv.both_op_execs++;
          }
...
#endif
#ifdef MUTATION_COUNT
              if (insert_at >= ARGV_MAX_SIZE) {
                afl->fsrv.file_op_execs++;
              } else if (insert_at + extra_len < ARGV_MAX_SIZE) {
                afl->fsrv.argv_op_execs++;
              } else {
                afl->fsrv.both_op_execs++;
              }
    ...
    if (afl->mut_argv_file_all) {
      ...
      afl->fsrv.is_argv_changed = !!memcmp(orig_in, out_buf, ARGV_MAX_SIZE);

      if (temp_len != afl->queue_cur->len) {
        afl->fsrv.is_file_changed = 1;
      } else {
        afl->fsrv.is_file_changed =
            !!memcmp(orig_in + ARGV_MAX_SIZE, out_buf + ARGV_MAX_SIZE,
...
abandon_entry:
  ...
  if (total_run > 0) {
    fprintf(
        afl->shrink_log, "%llu/%u (%0.2f)%% found\n",
