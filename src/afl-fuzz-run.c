...
fuzz_run_target(afl_state_t *afl, afl_forkserver_t *fsrv, u32 timeout) {
  ...
  u32 retry = 0;
  while ((res == FSRV_RUN_OOM) && (retry < 10)) {
    if (afl->stop_soon) { break; }
    WARNF("OOM detected, retrying...");
    fprintf(afl->debug_file, "Retrying forksrv OOM\n");
    ...
    res = afl_fsrv_run_target(fsrv, timeout, &afl->stop_soon);
    retry++;
  }

  if (retry == 10) { FATAL("OOM detected, giving up"); }
  ...
  while ((sd_ent = readdir(sd))) {
    ...
    for (o = m; o < n; o++) {
      ...
      if (st.st_size && st.st_size <= MAX_FILE) {
        ...
        afl->queued_imported +=
            save_if_interesting(afl, mem, st.st_size, fault, 0, 0);
        afl->syncing_party = 0;
...
u8 trim_case(afl_state_t *afl, struct queue_entry *q, u8 *in_buf) {
  ...
  if (afl->mut_argv_file_all) {
    if (orig_len <= ARGV_MAX_SIZE * TRIM_LIMIT) { return 0; }
  ...
  while (remove_len >= MAX(len_p2 / TRIM_END_STEPS, (u32)TRIM_MIN_BYTES)) {
    ...
    if (afl->mut_argv_file_all) {
      if (total_len - remove_len <= ARGV_MAX_SIZE * TRIM_LIMIT) {
        remove_len >>= 1;
        continue;
      }
    }
    ...
    while (remove_pos < total_len) {
      ...
      if (afl->mut_argv_file_all) {
        if ((total_len - trim_avail) <= ARGV_MAX_SIZE * TRIM_LIMIT) { break; }
