...
void create_alias_table(afl_state_t *afl) {
  ...
  if (likely(afl->schedule < RARE)) {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (!q->disabled && !(afl->mut_file_only && q->disabled_file_mut)) {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (afl->mut_file_only && q->disabled_file_mut) {
        q->weight = 0;
      } else if (!q->disabled) {
        double weight =
        ...
        if (!afl->mut_file_only) {
          weight = weight / ((double)afl->argv_to_queue_cnt[q->argv_id]);
        }
...
void add_to_queue(afl_state_t *afl, u8 *fname, u32 len, u8 passed_det, u8 *argv,
                  u32 argv_len, u8 *argv_fn) {
  ...
  q->argv = calloc(ARGV_MAX_SIZE, sizeof(u8));
  memcpy(q->argv, argv, argv_len);
  q->argv_len = argv_len;
  q->argv_fn = argv_fn;

  q->argv_id = get_argv_id(afl, q);
  q->file_id = get_file_id(afl);
...
u32 calculate_score(afl_state_t *afl, struct queue_entry *q) {
  ...
  switch (afl->schedule) {
    ...
    case COE:
      ...
      if ((q->fuzz_level_file + q->fuzz_level_argv) == 0) break;
    ...
    case LIN:
      factor = q->fuzz_level_file / (afl->n_fuzz[q->n_fuzz_entry] + 1);
      break;

    case QUAD:
      factor = q->fuzz_level_file * q->fuzz_level_file /
...
void log_queue_entry(afl_state_t *afl) {
  u8 file_fn[PATH_MAX];
  snprintf(file_fn, PATH_MAX, "%s/queue.log", afl->out_dir);
  FILE *fp = fopen(file_fn, "w");
  if (fp == NULL) { PFATAL("Unable to open '%s'", file_fn); }

  fprintf(
      fp,
      "queue_id:len:argv_id:argv_len:fuzz_level_argv:fuzz_level_file:depth\n");

  u32 idx1;
  for (idx1 = 0; idx1 < afl->queued_items; idx1++) {
    struct queue_entry *q = afl->queue_buf[idx1];
    fprintf(fp, "%u:%u:%u:%u:%u:%u:%llu\n", idx1, q->len, q->argv_id,
            q->argv_len, q->fuzz_level_argv, q->fuzz_level_file, q->depth);
  }
