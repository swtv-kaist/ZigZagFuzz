...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while ((opt = getopt(argc, argv,
                       "+Aa:b:B:c:CD:e:E:f:F:g:G:hi:I:j:K:l:L:m:M:nNOo:p:"
                       "Qq:Rs:S:t:T:UV:WXx:YZ")) > 0) {
    switch (opt) {
      case 'a':
      ...
      case 'q':
        ...
        // 0 - use both dict/rand mut on argv
  ...
  {
    u8 fn[PATH_MAX];
    snprintf(fn, PATH_MAX, "%s/debug.txt", afl->out_dir);
  ...
  argv[optind + 1] = strdup(afl->fsrv.out_argv);
  ...
  fprintf(afl->debug_file, "# of useless : %u/%u\n", afl->useless_at_start,
          afl->queued_items);
  ...
  afl->last_argv_file_crossover = afl->last_stage_change_time;

  afl->mut_file_only = 0;
  if (afl->mut_argv_file_all) { afl->mut_file_only = 1; }

  if (afl->old_out_dir) {
    get_func_rel(afl);
    if (afl->interleaving != 1) { shrink_corpus(afl); }
  }
  ...
  while (likely(!afl->stop_soon)) {
    ...
    if (unlikely((!afl->old_seed_selection &&
      ...
      //   if (afl->current_entry >= afl->queued_items) { afl->current_entry =
      //   0; }
    ...
    do {
      ...
      if (afl->mut_file_only) {
        ...
        if (afl->argv_mut_mode == 1) {
          ...
          if (rand_below(afl, 100) < DEFAULT_ARGV_RANDOM_MUT_PROB) {
            skipped_fuzz = fuzz_one_argv(afl);
          } else {
            skipped_fuzz = fuzz_one_comb_argv(afl);
          }
...
stop_fuzzing:
  ...
  if (afl->select_log) fclose(afl->select_log);
  if (afl->shrink_log) fclose(afl->shrink_log);
  if (afl->mut_log) fclose(afl->mut_log);
  if (afl->func_cov_log) fclose(afl->func_cov_log);
