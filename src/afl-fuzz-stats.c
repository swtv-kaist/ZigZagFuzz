...
void write_stats_file(afl_state_t *afl, u32 t_bytes, double bitmap_cvg,
  ...
  snprintf(fn, PATH_MAX, "%s/stage_finds", afl->out_dir);
  f = fopen(fn, "w");
  if (!f) { return; }

  fprintf(f, "havoc argv find : %llu/%llu : %0.2f%%\n"
    , afl->stage_finds[STAGE_HAVOC_ARGV], afl->stage_cycles[STAGE_HAVOC_ARGV],
    (double)afl->stage_finds[STAGE_HAVOC_ARGV] * 100 /
        (afl->stage_cycles[STAGE_HAVOC_ARGV] ? afl->stage_cycles[STAGE_HAVOC_ARGV]
                                              : 1));

  fprintf(f, "splice argv find : %llu/%llu : %0.2f%%\n\n"
    , afl->stage_finds[STAGE_SPLICE_ARGV], afl->stage_cycles[STAGE_SPLICE_ARGV]
    , (double)afl->stage_finds[STAGE_SPLICE_ARGV] * 100 /
        (afl->stage_cycles[STAGE_SPLICE_ARGV] ? afl->stage_cycles[STAGE_SPLICE_ARGV]
                                              : 1));

  fprintf(f, "havoc argv comb finds : %llu/%llu : %0.2f%%\n", afl->stage_finds[STAGE_ARGV_COMB],
          afl->stage_cycles[STAGE_ARGV_COMB],
          (double)afl->stage_finds[STAGE_ARGV_COMB] * 100 /
              (afl->stage_cycles[STAGE_ARGV_COMB] ? afl->stage_cycles[STAGE_ARGV_COMB]
                                                  : 1));
  ...
  fprintf(f, "havoc file find : %llu/%llu : %0.2f%%\n", afl->stage_finds[STAGE_HAVOC],
          ...
          (double)afl->stage_finds[STAGE_HAVOC] * 100 /
              (afl->stage_cycles[STAGE_HAVOC] ? afl->stage_cycles[STAGE_HAVOC] : 1));
  fprintf(f, "splice file find : %llu/%llu : %0.2f%%\n\n", afl->stage_finds[STAGE_SPLICE],
          ...
          (double)afl->stage_finds[STAGE_SPLICE] * 100 /
              (afl->stage_cycles[STAGE_SPLICE] ? afl->stage_cycles[STAGE_SPLICE] : 1));
  ...
  fprintf(f, "argv file crossover finds : %llu/%llu : %0.2f%%\n", afl->stage_finds[STAGE_ARGV_FILE_CROSS],
          ...
          (double)afl->stage_finds[STAGE_ARGV_FILE_CROSS] * 100 /
              (afl->stage_cycles[STAGE_ARGV_FILE_CROSS] ? afl->stage_cycles[STAGE_ARGV_FILE_CROSS]
                                                        : 1));
  fprintf(f, "force select finds : %llu/%llu : %0.2f%%\n", afl->stage_finds[FORCE_SELECT],
          afl->stage_cycles[FORCE_SELECT],
          (double)afl->stage_finds[FORCE_SELECT] * 100 /
              (afl->stage_cycles[FORCE_SELECT] ? afl->stage_cycles[FORCE_SELECT] : 1));
...
void maybe_update_plot_file(afl_state_t *afl, u32 t_bytes, double bitmap_cvg,
  ...
  fprintf(afl->fsrv.plot_file,
          ...
          afl->plot_prev_ed, t_bytes, afl->fsrv.argv_mut_execs, afl->fsrv.file_mut_execs,
          afl->fsrv.both_mut_execs, afl->fsrv.argv_op_execs, afl->fsrv.file_op_execs, afl->fsrv.both_op_execs); /* ignore errors */
...
#define SP20 SP10 SP10
  ...
  sprintf(tmp, "%s%s%u (%0.01f%%)", u_stringify_int(IB(0), afl->current_entry),
          afl->queue_cur->favored ? "." : "*", afl->queue_cur->fuzz_level_file,
  ...
  SAYF(bV bSTOP "  dictionary : " cRST "%-36s " bSTG bV bSTOP
       ...
       tmp,
       afl->sync_id ? u_stringify_int(IB(0), afl->num_argvs)
                    : (u8 *)"n/a");
