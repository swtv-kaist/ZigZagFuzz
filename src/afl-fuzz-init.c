  ...
  for (iter = 0; iter < afl->foreign_sync_cnt; iter++) {
    if (afl->foreign_syncs[iter].dir && afl->foreign_syncs[iter].dir[0]) {
      ...
      for (i = 0; i < (u32)nl_cnt; ++i) {
        ...
        afl->queued_imported += save_if_interesting(afl, mem, len, fault, 0, 0);
        afl->syncing_party = 0;
...
void read_testcases(afl_state_t *afl, u8 *directory, char **argv) {
  struct dirent **nl;
  ...
  // read default argv
  u8  default_argv[ARGV_MAX_SIZE];
  u32 argv_index = 0;
  u32 cur_argv_len = 0;
  memset(default_argv, 0, ARGV_MAX_SIZE);

  while (argv[argv_index] != NULL) {
    u32 arg_len = strlen(argv[argv_index]);

    if (unlikely(arg_len + cur_argv_len + 1 > ARGV_MAX_SIZE)) {
      FATAL("argv is too long, current max length is %d", ARGV_MAX_SIZE);
    }

    memcpy(default_argv + cur_argv_len, argv[argv_index], arg_len + 1);
    cur_argv_len += arg_len + 1;
    argv_index++;
  }

  if (unlikely(cur_argv_len + 1 > ARGV_MAX_SIZE)) {
    FATAL("argv is too long, current max length is %d", ARGV_MAX_SIZE);
  }
  ...
  if (nl_cnt) {
    ...
    do {
      ...
      add_to_queue(afl, fn2, st.st_size >= MAX_FILE ? MAX_FILE : st.st_size,
                   passed_det, default_argv, cur_argv_len, 0);
...
#endif /* ^!SIMPLE_FILES */
      u8 *argv_fn = alloc_printf("%s/queue_argvs/id:%06u", afl->out_dir, id);
      s32 argv_fd =
          open(argv_fn, O_WRONLY | O_CREAT | O_EXCL, DEFAULT_PERMISSION);
      if (argv_fd < 0) { PFATAL("Unable to create '%s'", argv_fn); }
      ck_write(argv_fd, q->argv, q->argv_len, argv_fn);
      close(argv_fd);
      q->argv_fn = argv_fn;
    }
...
static void handle_existing_out_dir(afl_state_t *afl) {
  ...
  fn = alloc_printf("%s/queue_argvs", afl->out_dir);
  if (delete_files(fn, CASE_PREFIX)) { goto dir_cleanup_failed; }
  ck_free(fn);

  fn = alloc_printf("%s/crashes_argvs", afl->out_dir);
  if (delete_files(fn, CASE_PREFIX)) { goto dir_cleanup_failed; }
  ck_free(fn);

  fn = alloc_printf("%s/hangs_argvs", afl->out_dir);
  if (delete_files(fn, CASE_PREFIX)) { goto dir_cleanup_failed; }
  ck_free(fn);

  fn = alloc_printf("%s/power", afl->out_dir);
  if (delete_files(fn, NULL)) { goto dir_cleanup_failed; }
  ck_free(fn);
  ...
  fn = alloc_printf("%s/.cur_argv", afl->tmp_dir);
  if (unlink(fn) && errno != ENOENT) { goto dir_cleanup_failed; }
  ck_free(fn);
...
void setup_dirs_fds(afl_state_t *afl) {
  ...
  if (!afl->in_place_resume) {
    ...
    fprintf(afl->fsrv.plot_file,
            ...
            "argv_mut_exec, file_mut_exec, both_mut_exec, argv_mut_op, "
            "file_mut_op, both_mut_op\n");
  ...
  tmp = alloc_printf("%s/queue_argvs", afl->out_dir);
  if (mkdir(tmp, 0700)) { PFATAL("Unable to create '%s'", tmp); }
  ck_free(tmp);

  tmp = alloc_printf("%s/crashes_argvs", afl->out_dir);
  if (mkdir(tmp, 0700)) { PFATAL("Unable to create '%s'", tmp); }
  ck_free(tmp);

  tmp = alloc_printf("%s/hangs_argvs", afl->out_dir);
  if (mkdir(tmp, 0700)) { PFATAL("Unable to create '%s'", tmp); }
  ck_free(tmp);

  tmp = alloc_printf("%s/power", afl->out_dir);
  if (mkdir(tmp, 0700)) { PFATAL("Unable to create '%s'", tmp); }
  ck_free(tmp);
...
void setup_fsrv_out_argv(afl_state_t *afl) {
  afl->fsrv.out_argv = alloc_printf("%s/.cur_argv", afl->tmp_dir);

  unlink(afl->fsrv.out_argv); /* Ignore errors */
}
