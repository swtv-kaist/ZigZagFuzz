...
static void write_gen_argvs(afl_state_t *, u32, u32 **, u32 *);
static void write_shrink_log_after(afl_state_t *);
static float get_jaccard_dist(u32 *func_calls1, u32 func_cnt1, u32 *func_calls2,
                              u32 func_cnt2);

void read_argv_keywords(afl_state_t *afl) {
  if (afl->keyword_fn == NULL) {
    PFATAL("No keyword file specified (-a)");
  }

  FILE *f = fopen(afl->keyword_fn, "r");
  if (!f) {
    PFATAL("Unable to open '%s'", afl->keyword_fn);
  }
        ...
        default:
          ...
          if (afl->extras_cnt) {
            if (r < 2) {
              ...
              u32 use_extra = rand_below(afl, afl->extras_cnt);
              u32 extra_len = afl->extras[use_extra].len;
              if (temp_len + extra_len >= ARGV_MAX_SIZE) {
                break;
              }
...
u8 fuzz_one_comb_argv(afl_state_t *afl) {
  ...
  for (idx = 0; idx < len - 1; idx++) {
    if (in_buf[idx] == 0) {
      ...
      if (unlikely(orig_argv_len >= MAX_ARGV_WORDS)) {
        goto abandon_entry;
      }
    }
  }
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    for (i = 0; i < use_stacking; ++i) {
      switch ((r = rand_below(afl, 3))) {
        case 0:  // word addition
          if (cur_argv_len + 1 >= MAX_ARGV_WORDS) {
            break;
          }
    ...
    for (idx = 0; idx < cur_argv_len; idx++) {
      ...
      if (unlikely(cur_len + word_len + 1 >= ARGV_MAX_SIZE)) {
        break;
      }
...
u8 fuzz_one_concat_comb(afl_state_t *afl) {
  u32 total_len;
  u32 i, idx;
  u8 *in_buf, *out_buf = 0;
  u64 havoc_queued = 0, orig_hit_cnt, new_hit_cnt = 0;
  ...
  if (unlikely(afl->not_on_tty)) {
    ACTF(
        "Fuzzing test case #%u comb argv (%u total, %llu crashes saved, "
        "perf_score=%0.0f, exec_us=%llu, hits=%u, map=%u, ascii=%u)...",
        afl->current_entry, afl->queued_items, afl->saved_crashes,
        afl->queue_cur->perf_score, afl->queue_cur->exec_us,
        likely(afl->n_fuzz) ? afl->n_fuzz[afl->queue_cur->n_fuzz_entry] : 0,
  ...
  total_len = afl->queue_cur->len + ARGV_MAX_SIZE;
  ...
  afl->stage_name = "havoc_argv_ccomb";
  afl->stage_short = "havoc_argv_ccomb";
  afl->stage_max = (doing_det ? HAVOC_CYCLES_INIT : HAVOC_CYCLES) * perf_score /
                   afl->havoc_div / 100;
  ...
  u8 *cur_ptr = in_buf;
  for (idx = 0; idx < ARGV_MAX_SIZE; idx++) {
    if (in_buf[idx] == 0) {
      if (in_buf[idx + 1] == 0) {
        break;
      }
  ...
  memcpy(out_buf, in_buf, total_len);
  u32 cur_len = 0, cur_argv_len = orig_argv_len;
  u8 r;
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    if (common_fuzz_stuff(afl, out_buf, total_len, 0, 0)) {
      goto abandon_entry;
    }
...
u8 fuzz_one_concat_argv(afl_state_t *afl) {
  u32 temp_len;
  u32 i;
  u8 *in_buf, *out_buf;
  u64 havoc_queued = 0, orig_hit_cnt, new_hit_cnt = 0;
  ...
  in_buf = queue_testcase_get(afl, afl->queue_cur);
  const s32 total_len = afl->queue_cur->len + ARGV_MAX_SIZE;

  out_buf = afl_realloc(AFL_BUF_PARAM(out), total_len);
  if (unlikely(!out_buf)) {
    PFATAL("alloc");
  }
...
havoc_stage:
  ...
  if (!splice_cycle) {
    afl->stage_name = "havoc_cargv";
    afl->stage_short = "havoc_cargv";
    afl->stage_max = (doing_det ? HAVOC_CYCLES_INIT : HAVOC_CYCLES) *
                     perf_score / afl->havoc_div / 100;
...
#define FLIP_BIT(_ar, _b)                     \
      ...
      switch ((r = rand_below(afl, r_max))) {
        ...
        case 44 ... 46: {
          if (temp_len + HAVOC_ARGV_BLK_XL < ARGV_MAX_SIZE) {
            ...
            u8 *new_buf = afl_realloc(AFL_BUF_PARAM(out_scratch), total_len);
            if (unlikely(!new_buf)) {
              PFATAL("alloc");
            }
            ...
            memcpy(new_buf + ARGV_MAX_SIZE, out_buf + ARGV_MAX_SIZE,
                   total_len - ARGV_MAX_SIZE);
        ...
        default:
          ...
          if (afl->extras_cnt) {
            if (r < 2) {
              ...
              out_buf = new_buf;
              afl_swap_bufs(AFL_BUF_PARAM(out_scratch), AFL_BUF_PARAM(out));
              temp_len += extra_len;
          ...
          /* Get the testcase for splicing. */
          struct queue_entry *target = afl->queue_buf[tid];
          u32 new_len = target->len + ARGV_MAX_SIZE;
          ...
          if ((temp_len >= 2 && r % 2) ||
            ...
            u32 clone_from, clone_to, clone_len;
            u32 clone_max = ARGV_MAX_SIZE - temp_len - 2;

            clone_len = choose_block_len_small(afl, MIN(clone_max, new_len));
            clone_from = rand_below(afl, new_len - clone_len + 1);
            clone_to = rand_below(afl, temp_len + 1);

            u8 *temp_buf = afl_realloc(AFL_BUF_PARAM(out_scratch), total_len);
            if (unlikely(!temp_buf)) {
              PFATAL("alloc");
            }
            ...
            memcpy(temp_buf + ARGV_MAX_SIZE, out_buf + ARGV_MAX_SIZE,
                   total_len - ARGV_MAX_SIZE);
  ...
  if (!splice_cycle) {
    afl->stage_finds[STAGE_HAVOC_ARGV] += new_hit_cnt - orig_hit_cnt;
    afl->stage_cycles[STAGE_HAVOC_ARGV] += afl->stage_max;

  } else {
    afl->stage_finds[STAGE_SPLICE_ARGV] += new_hit_cnt - orig_hit_cnt;
    afl->stage_cycles[STAGE_SPLICE_ARGV] += afl->stage_max;
  }
...
retry_splicing:
  ...
  if (afl->use_splicing && splice_cycle++ < SPLICE_CYCLES &&
    ...
    locate_diffs(in_buf, new_buf, ARGV_MAX_SIZE, &f_diff, &l_diff);
    ...
    afl->in_scratch_buf = afl_realloc(AFL_BUF_PARAM(in_scratch), total_len);
    memcpy(afl->in_scratch_buf, in_buf, split_at);
    memcpy(afl->in_scratch_buf + split_at, new_buf, ARGV_MAX_SIZE - split_at);
    memcpy(afl->in_scratch_buf + ARGV_MAX_SIZE, in_buf + ARGV_MAX_SIZE,
           total_len - ARGV_MAX_SIZE);
    in_buf = afl->in_scratch_buf;
    afl_swap_bufs(AFL_BUF_PARAM(in), AFL_BUF_PARAM(in_scratch));
...
void select_argv(afl_state_t *afl) {
  ...
  if (fp == NULL) {
    PFATAL("Unable to create '%s'", file_fn);
  }
  ...
  fprintf(afl->debug_file, "argv selection time : %llums\n",
...
void read_old_argv_dir(afl_state_t *afl) {
  u8 *plot_fn = alloc_printf("%s/plot_data", afl->old_out_dir);

  FILE *plot_f = fopen(plot_fn, "r");
  if (!plot_f) {
    PFATAL("Unable to open '%s'", plot_fn);
  }
  ck_free(plot_fn);

  u8 buffer[512];
  u32 idx2, colon_cnt, str_idx = 0, comma_cnt;
  s32 cur_time;
  u32 num_input_to_read = 0;

  if (fgets(buffer, 512, plot_f)) {
  }
  while (fgets(buffer, 512, plot_f)) {
    comma_cnt = 0;
    idx2 = 0;
    ...
    for (idx2 = 0; idx2 < 512; idx2++) {
      if (buffer[idx2] == ',') {
        ...
        if (comma_cnt == 1) {
          ...
          buffer[idx2] = 0;
          num_input_to_read = atoi(buffer + str_idx);
        }
      } else if (buffer[idx2] == 0) {
        break;
      }
    }

    if (num_input_to_read != 0) {
      break;
    }
  }

  fprintf(afl->debug_file, "# of input to read : %u\n", num_input_to_read);

  if (unlikely(num_input_to_read == 0)) {
    PFATAL("Can't get old inputs");
  }

  u8 *queue_argv_dir = alloc_printf("%s/queue_argvs", afl->old_out_dir);
  u8 *queue_dir = alloc_printf("%s/queue", afl->old_out_dir);
  u8 *queue_log_fn = alloc_printf("%s/queue.log", afl->old_out_dir);
  FILE *queue_log_f = fopen(queue_log_fn, "r");
  if (!queue_log_f) {
    PFATAL("Unable to open '%s'", queue_log_fn);
  }
  ck_free(queue_log_fn);

  u32 queue_size = 2048;
  u32 queue_idx = 0;
  u32 *queue_num_file_fuzzed = (u32 *)calloc(queue_size, sizeof(u32));

  if (fgets(buffer, 512, queue_log_f)) {
  }
  while (fgets(buffer, 512, queue_log_f)) {
    idx2 = 0;
    colon_cnt = 0;
    str_idx = 0;
    for (idx2 = 0; idx2 < 512; idx2++) {
    ...
    if (queue_idx >= num_input_to_read) {
      break;
    }

    if (unlikely(queue_idx >= queue_size)) {
      queue_size *= 2;
      queue_num_file_fuzzed =
          realloc(queue_num_file_fuzzed, queue_size * sizeof(u32));
    }
    queue_num_file_fuzzed[queue_idx++] = atoi(buffer + str_idx);
  }

  fclose(queue_log_f);

  struct dirent **argv_nl;

  s32 argv_cnt = scandir(queue_argv_dir, &argv_nl, NULL, alphasort);

  if (argv_cnt <= 0 || queue_argv_dir == NULL) {
    PFATAL("Unable to find files in '%s'", queue_argv_dir);
  }

  struct dirent **file_nl;

  s32 file_cnt = scandir(queue_dir, &file_nl, NULL, alphasort);

  if (file_cnt <= 0 || queue_dir == NULL) {
    PFATAL("Unable to find files in '%s'", queue_argv_dir);
  }
  ...
  while (idx1 > 0) {
    ...
    struct stat st;
    u8 *argv_fn = alloc_printf("%s/%s", queue_argv_dir, argv_nl[idx1]->d_name);

    u32 tc_id = atoi(argv_nl[idx1]->d_name + 3);

    if (tc_id >= num_input_to_read) {
      ck_free(argv_fn);
      free(argv_nl[idx1]);
      continue;
    }

    if (lstat(argv_fn, &st) || access(argv_fn, R_OK)) {
      PFATAL("Unable to access '%s'", argv_fn);
    }

    if (!S_ISREG(st.st_mode) || (st.st_size < 3)) {
      ck_free(argv_fn);
      free(argv_nl[idx1]);
      continue;
    }

    int argv_fd = open(argv_fn, O_RDONLY);
    if (argv_fd < 0) {
      PFATAL("Unable to open '%s'", argv_fn);
    }

    u8 *argv_buf = malloc(ARGV_MAX_SIZE);
    u32 argv_len = st.st_size;

    ck_read(argv_fd, argv_buf, argv_len, argv_fn);
    close(argv_fd);
    ck_free(argv_fn);
    ...
    while (idx2 > 0) {
      ...
      if (strncmp(argv_nl[idx1]->d_name, file_nl[idx2]->d_name, 9)) {
        continue;
      }

      u8 *file_fn = alloc_printf("%s/%s", queue_dir, file_nl[idx2]->d_name);

      struct stat st;

      if (lstat(file_fn, &st) || access(file_fn, R_OK)) {
        PFATAL("Unable to access '%s'", file_fn);
      }
      ...
      add_to_queue(afl, file_fn, st.st_size >= MAX_FILE ? MAX_FILE : st.st_size,
                   0, argv_buf, argv_len, 0);
      afl->queue_top->fuzz_level_file = queue_num_file_fuzzed[tc_id];
      break;
    }
  ...
  ck_free(queue_argv_dir);
  ck_free(queue_dir);
  free(argv_nl);
  free(queue_num_file_fuzzed);
...
u32 select_next_queue_entry_argv(afl_state_t *afl) {
  u32 idx1;
  fprintf(afl->select_log, "# of argvs : %u\n", afl->num_argvs);

  for (idx1 = afl->last_checked_argv; idx1 < afl->num_argvs; idx1++) {
    u32 num_queue = afl->argv_to_queue_cnt[idx1];
    if (num_queue > 1) {
      continue;
    }
    u32 queue_idx = afl->argv_to_queue[idx1][0];
    struct queue_entry *q = afl->queue_buf[queue_idx];
    if (q->fuzz_level_file) {
      continue;
    }

    u32 selected_queue_idx = afl->argv_to_queue[idx1][0];
    fprintf(afl->select_log, "Selected cmd %u : queue : %u\n", idx1,
            selected_queue_idx);

    afl->last_checked_argv = idx1 + 1;
    return selected_queue_idx;
  }

  afl->last_checked_argv = afl->num_argvs;

  fprintf(afl->select_log, "All %u cmds are fuzzed\n", afl->num_argvs);

  return select_next_queue_entry(afl);
}

static void write_gen_argvs(afl_state_t *afl, u32 num_argvs,
                            u32 **argv_to_queue, u32 *num_argv_to_queue) {
  u8 file_fn[PATH_MAX];
  snprintf(file_fn, PATH_MAX, "%s/power/gen_argvs", afl->out_dir);
...
void init_argv_buf(afl_state_t *afl) {
  ...
  u8 file_fn[PATH_MAX];
  snprintf(file_fn, PATH_MAX, "%s/power/select_log", afl->out_dir);
  afl->select_log = fopen(file_fn, "w");
  if (!afl->select_log) {
    PFATAL("Unable to create '%s'", file_fn);
  }

  snprintf(file_fn, PATH_MAX, "%s/power/shrink_log", afl->out_dir);
  afl->shrink_log = fopen(file_fn, "w");
  if (!afl->shrink_log) {
    PFATAL("Unable to create '%s'", file_fn);
  }

  snprintf(file_fn, PATH_MAX, "%s/power/func_cov_log", afl->out_dir);
  afl->func_cov_log = fopen(file_fn, "w");
  if (!afl->func_cov_log) {
    PFATAL("Unable to create '%s'", file_fn);
  }
}
...
u32 get_argv_id(afl_state_t *afl, struct queue_entry *q) {
  ...
  for (idx1 = 0; idx1 < num_argvs; idx1++) {
    ...
    if (argv1_len != new_argv_len) {
      continue;
    }
    if (memcmp(argv1, new_argv, new_argv_len) != 0) {
      continue;
    }
    break;
  }
...
void argv_file_crossover(afl_state_t *afl) {
  u32 num_argvs = afl->num_argvs;

  afl->stage_name = "argv_file_crossover";
  afl->stage_short = "argv_file_cross";
  afl->stage_max = num_argvs;
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; afl->stage_cur++) {
    u32 num_argv_to_queue = afl->argv_to_queue_cnt[afl->stage_cur];

    if (num_argv_to_queue > 100) {
      continue;
    }
    struct queue_entry *argv_queue =
        afl->queue_buf[afl->argv_to_queue[afl->stage_cur][0]];
    if (argv_queue->disabled_file_mut) {
      continue;
    }

    u8 *argv_buf = argv_queue->argv;
    u32 argv_len = argv_queue->argv_len;

    u32 idx2;

    for (idx2 = 0; idx2 < 1024; idx2++) {
      u32 queue_idx = rand_below(afl, afl->queued_items);
      struct queue_entry *file_q = afl->queue_buf[queue_idx];

      u8 *file_buf = queue_testcase_get(afl, file_q);
      u32 file_len = file_q->len;

      if (unlikely(
              common_fuzz_stuff(afl, file_buf, file_len, argv_buf, argv_len))) {
        continue;
      }
      num_exec++;
    }
  ...
  u64 new_hit_cnts = afl->queued_items + afl->saved_crashes - orig_hit_cnt;

  afl->stage_finds[STAGE_ARGV_FILE_CROSS] += new_hit_cnts;
  afl->stage_cycles[STAGE_ARGV_FILE_CROSS] += num_exec;
...
void shrink_corpus(afl_state_t *afl) {
  // shrink
  u32 idx1, idx2, idx3, queue_idx, argv_idx, active_idx;
  struct queue_entry *q;

  u32 num_queue;
  u32 num_onefile_argv = 0;
  ...
  fprintf(afl->shrink_log, "# of file mutated last period: %u, ",
          afl->num_selected_file_for_mut);

  fprintf(afl->shrink_log, "# of tcs : %u, # of active tcs: %u\n",
          afl->queued_items, num_active_tcs);
  fprintf(afl->shrink_log,
          "# of argvs : %u, # onefile argv: %u, # Active Argvs : %u\n",
          afl->num_argvs, num_onefile_argv,
          num_active_zero_argvs + num_good_argvs);
  fprintf(
      afl->shrink_log,
      "# of unique files : %u, # of one argv files : %u, # active files: %u\n",
      afl->num_unique_files, num_oneargv_files, num_active_files);

  if (num_active_zero_argvs < NUM_ARGV_CLUSTERS * NUM_SEL_IN_ARGV_CLUSTER) {
    free(active_zero_argvs);
    free(good_argvs);
    fprintf(afl->shrink_log, "Not enough argvs to do clustering, skip\n");
    write_shrink_log_after(afl);
    return;
  }

  u32 num_active_argvs = num_active_zero_argvs + num_good_argvs;

  u32 *selected_argvs = (u32 *)malloc(sizeof(u32) * num_active_argvs);

  u64 time_begin = get_cur_time();
  u32 **func_calls = (u32 **)calloc(num_active_argvs, sizeof(u32 *));
  u32 *func_call_cnts = (u32 *)calloc(num_active_argvs, sizeof(u32));
  u8 *func_map = afl->shm.func_map;
  u8 *in_buf = 0;

  for (idx1 = 0; idx1 < num_active_zero_argvs; idx1++) {
    argv_idx = active_zero_argvs[idx1];
  ...
  u32 *num_argvs_cluster = (u32 *)calloc(NUM_ARGV_CLUSTERS, sizeof(u32));
  u8 *selected_argvs_bits = (u8 *)calloc(num_active_zero_argvs, sizeof(u8));

  // 1. pick random centroids
  for (idx1 = 0; idx1 < NUM_ARGV_CLUSTERS; idx1++) {
    u32 rand_argv_idx = rand_below(afl, num_active_zero_argvs);
    while (selected_argvs_bits[rand_argv_idx]) {
      rand_argv_idx = rand_below(afl, num_active_zero_argvs);
    }
    selected_argvs_bits[rand_argv_idx] = 1;
    clusters[idx1] = (u32 *)malloc(sizeof(u32) * num_active_zero_argvs);
    clusters[idx1][0] = rand_argv_idx;
  }

  u8 centroid_changed = 1;
  u32 num_iteration = 0;
  while (centroid_changed && (num_iteration++ < 20) && (afl->stop_soon == 0)) {
    // 2. assign argvs to clusters
    centroid_changed = 0;

    memset(selected_argvs_bits, 0, sizeof(u8) * num_active_zero_argvs);
    for (idx1 = 0; idx1 < NUM_ARGV_CLUSTERS; idx1++) {
      selected_argvs_bits[clusters[idx1][0]] = 1;
      num_argvs_cluster[idx1] = 1;
    }

    for (idx1 = 0; idx1 < num_active_zero_argvs; idx1++) {
      if (selected_argvs_bits[idx1]) {
        continue;
      }
      ...
      clusters[min_dist_cluster][num_argvs_cluster[min_dist_cluster]++] = idx1;
    }
    ...
    for (idx1 = 0; idx1 < NUM_ARGV_CLUSTERS; idx1++) {
      ...
      for (idx2 = 1; idx2 < num_argvs_cluster[idx1]; idx2++) {
      ...
      for (idx2 = 1; idx2 < num_argvs_cluster[idx1]; idx2++) {
        ...
        for (idx3 = 0; idx3 < num_argvs_cluster[idx1]; idx3++) {
          if (idx2 == idx3) {
            continue;
          }
  ...
  u32 *rank_vals = (u32 *)malloc(sizeof(u32) * num_active_zero_argvs);
  u32 *rank_idxes = (u32 *)malloc(sizeof(u32) * num_active_zero_argvs);

  for (idx1 = 0; idx1 < NUM_ARGV_CLUSTERS; idx1++) {
    fprintf(afl->shrink_log, "shrink cluster %u:", idx1);
    u32 *cluster = clusters[idx1];
    u32 num_argv_in_cluster = num_argvs_cluster[idx1];

    if (num_argv_in_cluster < NUM_SEL_IN_ARGV_CLUSTER) {
      for (idx2 = 0; idx2 < num_argv_in_cluster; idx2++) {
        active_idx = cluster[idx2];
        argv_idx = active_zero_argvs[active_idx];
        selected_argvs[num_selected++] = argv_idx;
        fprintf(afl->shrink_log, "%u,", argv_idx);
      }
      fprintf(afl->shrink_log, "\n");
      continue;
    }

    memset(rank_vals, 0, sizeof(float) * num_argv_in_cluster);
    ...
    for (idx2 = 0; idx2 < num_argv_in_cluster; idx2++) {
      ...
      for (idx3 = 0; idx3 < idx2; idx3++) {
        if (rank_vals[idx3] <= avg_score) {
          memmove(&rank_vals[idx3 + 1], &rank_vals[idx3],
                  sizeof(float) * (num_argv_in_cluster - idx3 - 1));
          memmove(&rank_idxes[idx3 + 1], &rank_idxes[idx3],
                  sizeof(u32) * (num_argv_in_cluster - idx3 - 1));
          rank_vals[idx3] = avg_score;
          rank_idxes[idx3] = argv_idx;
          break;
        }
      }

      if (idx3 == idx2) {
        rank_vals[idx2] = avg_score;
        rank_idxes[idx2] = argv_idx;
      }
    }
    ...
    for (idx2 = 0; idx2 < NUM_SEL_IN_ARGV_CLUSTER; idx2++) {
      selected_argvs[num_selected++] = rank_idxes[idx2];
    }
  }

  memcpy(selected_argvs + num_selected, good_argvs,
         sizeof(u32) * num_good_argvs);
  num_selected += num_good_argvs;

  for (idx1 = 0; idx1 < num_active_argvs; idx1++) {
    free(func_calls[idx1]);
  }
  free(func_calls);
  free(func_call_cnts);
  ...
  for (idx1 = 0; idx1 < num_selected; idx1++) {
    argv_idx = selected_argvs[idx1];
    num_queue = afl->argv_to_queue_cnt[argv_idx];
    for (queue_idx = 0; queue_idx < num_queue; queue_idx++) {
      selected_queue_bits[afl->argv_to_queue[argv_idx][queue_idx]] = 1;
    }
  }

  for (idx1 = 0; idx1 < afl->queued_items; idx1++) {
    if (selected_queue_bits[idx1]) {
      continue;
    }
  ...
  fprintf(afl->shrink_log, "SHRINK took %llums\n", time_passed);
...
void shrink3(afl_state_t *afl) {
  // Remove zero argvs randomly
  u32 idx1, queue_idx, argv_idx;
  struct queue_entry *q;
...
void write_shrink_log(afl_state_t *afl) {
  ...
  fprintf(afl->shrink_log, "Shrink #%u\n", shrink_log_index++);
  ...
  for (idx1 = 0; idx1 < afl->num_argvs; idx1++) {
    ...
    fprintf(afl->shrink_log, "SHRINK res1: Argv #%u : %u/%u active\n", idx1,
  ...
  for (idx1 = 0; idx1 < afl->num_unique_files; idx1++) {
    ...
    fprintf(afl->shrink_log, "SHRINK res2: File #%u : %u/%u active\n", idx1,
            num_active, num_queue);
  }

  fprintf(afl->shrink_log, "finish1\n");
  fflush(afl->shrink_log);
}

void write_mut_log(afl_state_t *afl) {
  if (afl->mut_log == 0) {
    u8 file_fn[PATH_MAX];
    snprintf(file_fn, PATH_MAX, "%s/power/mut_log", afl->out_dir);
    afl->mut_log = fopen(file_fn, "w");
  }

  static u32 mut_log_index = 0;

  u32 idx1;
  fprintf(afl->mut_log, "Log#%u\n", mut_log_index++);
  for (idx1 = 0; idx1 < afl->num_argvs; idx1++) {
    fprintf(afl->mut_log, "arg,%u,%llu,%u,%llu\n", afl->argv_num_finds[idx1],
            afl->argv_num_mut[idx1], afl->argv_num_file_finds[idx1],
            afl->argv_num_file_mut[idx1]);
  }

  for (idx1 = 0; idx1 < afl->num_unique_files; idx1++) {
    fprintf(afl->mut_log, "file,%u,%llu,%u,%llu\n", afl->file_num_finds[idx1],
            afl->file_num_mut[idx1], afl->file_num_argv_finds[idx1],
            afl->file_num_argv_mut[idx1]);
  }

  fprintf(afl->mut_log, "finish\n");
  fflush(afl->mut_log);
}

void log_fr(afl_state_t *afl) {
  u32 idx1;
  FILE *fp;
  u8 filefn[PATH_MAX];
  // snprintf(filefn, PATH_MAX, "%s/power/func_exec", afl->out_dir);
  // fp = fopen(filefn, "w");
  // if (fp == NULL) { PFATAL("Unable to create '%s'", filefn);}

  // for (idx1 = 0; idx1 < FUNC_MAP_SIZE; idx1++) {

  //   if (afl->func_exec_map[idx1] != 0) {
  //     fprintf(fp, "0");
  //     fwrite(afl->func_exec_map[idx1], sizeof(u32), FUNC_MAP_SIZE, fp);
  //   } else {
  //     fprintf(fp, "1");
  //   }
  // }
  ...
  snprintf(filefn, PATH_MAX, "%s/power/last_mut_log", afl->out_dir);
  fp = fopen(filefn, "w");

  for (idx1 = 0; idx1 < afl->num_argvs; idx1++) {
    fprintf(fp, "arg,%u,%llu,%u,%llu\n", afl->argv_num_finds[idx1],
            afl->argv_num_mut[idx1], afl->argv_num_file_finds[idx1],
            afl->argv_num_file_mut[idx1]);
  }

  for (idx1 = 0; idx1 < afl->num_unique_files; idx1++) {
    fprintf(fp, "file,%u,%llu,%u,%llu\n", afl->file_num_finds[idx1],
            afl->file_num_mut[idx1], afl->file_num_argv_finds[idx1],
            afl->file_num_argv_mut[idx1]);
  }
...
static void file_clustering(afl_state_t *afl, u32 *zero_files,
  ...
  for (idx1 = 0; idx1 < NUM_FILE_CLUSTERS; idx1++) {
    fprintf(afl->shrink_log, "shrink file cluster %u:", idx1);
    u32 *cluster = clusters[idx1];
    ...
    for (idx2 = 0; idx2 < num_file_in_cluster; idx2++) {
      ...
      fprintf(afl->shrink_log, "%u,", file_idx);
...
void shrink_corpus2(afl_state_t *afl) {
  // shrink
  ...
  u32 num_min_selected_tcs = afl->num_selected_file_for_mut * NUM_SEL_RATIO;

  afl->num_selected_file_for_mut = 0;

  fprintf(afl->shrink_log, "# of tcs : %u, # of active tcs: %u\n",
          afl->queued_items, num_active_tcs);
  fprintf(afl->shrink_log, "# of argvs : %u, # Active Argvs : %u\n",
          afl->num_argvs, num_active_zero_argvs + num_good_argvs);
  fprintf(afl->shrink_log, "# of unique files : %u, # active files: %u\n",
  ...
  for (idx1 = 0; idx1 < NUM_ARGV_CLUSTERS; idx1++) {
    for (idx2 = 0; idx2 < NUM_SEL_IN_ARGV_CLUSTER; idx2++) {
      if (idx2 >= cluster_num_argvs[idx1]) {
        break;
      }
  ...
  for (idx1 = 0; idx1 < NUM_FILE_CLUSTERS; idx1++) {
    for (idx2 = 0; idx2 < NUM_SEL_IN_FILE_CLUSTER; idx2++) {
      if (idx2 >= cluster_num_files[idx1]) {
        break;
      }
  ...
  for (idx1 = 0; idx1 < afl->queued_items; idx1++) {
    ...
    if (!selected_argvs_bits[q->argv_id]) {
      continue;
    }
    if (!selected_files_bits[q->file_id]) {
      continue;
    }
...
static void write_shrink_log_after(afl_state_t *afl) {
  ...
  for (idx1 = 0; idx1 < afl->num_argvs; idx1++) {
    ...
    fprintf(afl->shrink_log, "SHRINK res3: Argv #%u : %u/%u active\n", idx1,
            num_active, num_queue);
  }
  ...
  for (idx1 = 0; idx1 < afl->num_unique_files; idx1++) {
    ...
    fprintf(afl->shrink_log, "SHRINK res4: File #%u : %u/%u active\n", idx1,
            num_active, num_queue);
  }

  fprintf(afl->shrink_log, "SHIRNK sum : %u/%u active, # active argv : %u\n",
          num_active_total, afl->queued_items, num_active_argvs);
  fprintf(afl->shrink_log, "finish2\n");
  fflush(afl->shrink_log);
}
