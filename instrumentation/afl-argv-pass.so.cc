...
#if LLVM_VERSION_MAJOR >= 11 /* use new pass manager */
PreservedAnalyses ArgvTransform::run(Module &M, ModuleAnalysisManager &MAM) {
#else
...
static RegisterStandardPasses RegisterArgvTransPassLTO(
    PassManagerBuilder::EP_FullLinkTimeOptimizationLast, registerArgvTransPass);
  #endif
#endif

void ArgvTransform::insert_argv(Function *main_func) {
  IRB->SetInsertPoint(
      main_func->getEntryBlock().getFirstNonPHIOrDbgOrLifetime());

  if (main_func->arg_size() != 2) { return; }
...
void ArgvTransform::replace_open_funcs() {
  Value *fopen_wrapperHook =
      Mod->getOrInsertFunction("__afl_fopen_wrapper", FilePtrTy, Int8PtrTy,
                               Int8PtrTy)
          .getCallee();
  Value *freopen_wrapperHook =
      Mod->getOrInsertFunction("__afl_freopen_wrapper", FilePtrTy, Int8PtrTy,
                               Int8PtrTy, FilePtrTy)
          .getCallee();
  FunctionCallee open_wrapperHookFC = Mod->getOrInsertFunction(
      "__afl_open_wrapper", Int32Ty, Int8PtrTy, Int32Ty);
  Value        *open_wrapperHook = open_wrapperHookFC.getCallee();
  FunctionType *open_ft =
      FunctionType::get(Int32Ty, {Int8PtrTy, Int32Ty}, true);
  FunctionCallee open_wrapperHookFC2 =
      Mod->getOrInsertFunction("__afl_open_wrapper2", open_ft);
  Value *open_wrapperHook2 = open_wrapperHookFC2.getCallee();
  Value *creat_wrapperHook =
      Mod->getOrInsertFunction("__afl_creat_wrapper", Int32Ty, Int8PtrTy,
                               Int32Ty)
          .getCallee();
  Value *mkstemp_wrapperHook =
      Mod->getOrInsertFunction("__afl_mkstemp_wrapper", Int32Ty, Int8PtrTy)
          .getCallee();
  Value *mkstemps_wrapperHook =
      Mod->getOrInsertFunction("__afl_mkstemps_wrapper", Int32Ty, Int8PtrTy,
                               Int32Ty)
          .getCallee();
  // Value * mkostemp_wrapperHook =
  // Mod->getOrInsertFunction("__afl_mkostemp_wrapper", Int32Ty, Int8PtrTy,
  // Int32Ty).getCallee(); Value * mkostemps_wrapperHook =
  // Mod->getOrInsertFunction("__afl_mkostemps_wrapper", Int32Ty, Int8PtrTy,
  // Int32Ty, Int32Ty).getCallee(); Value * mkdtemp_wrapperHook =
  // Mod->getOrInsertFunction("__afl_mkdtemp_wrapper", Int8PtrTy,
  // Int8PtrTy).getCallee();
    ...
    if (func_name == "open") {
      ...
      if (ft->getNumParams() >= 2 && ft->getReturnType()->isIntegerTy(32) &&
        ...
        if (ft->isVarArg()) {
          ...
          for (auto iter = open_wrapperHook->user_begin();
               iter != open_wrapperHook->user_end(); iter++) {
            Value *user = *iter;
      ...
      FunctionType *ft = F.getFunctionType();
      if (ft->getNumParams() == 2 && ft->getReturnType()->isPointerTy() &&
          ft->getParamType(0)->isPointerTy() &&
      ...
      FunctionType *ft = F.getFunctionType();
      if (ft->getNumParams() == 3 && ft->getReturnType()->isPointerTy() &&
          ft->getParamType(0)->isPointerTy() &&
          ft->getParamType(1)->isPointerTy() &&
...
void ArgvTransform::insert_func_probe() {
  Constant *func_map_ptr =
      Mod->getOrInsertGlobal("__afl_func_map_ptr", Int8PtrTy);
  ...
  for (auto &F : Mod->functions()) {
    ...
    Value *map_idx_ptr = IRB->CreateInBoundsGEP(
        Int8Ty, func_map_ptr_val, ConstantInt::get(Int32Ty, func_id));
    IRB->CreateStore(ConstantInt::get(Int8Ty, 1), map_idx_ptr);

    func_id++;

    if (func_id >= FUNC_MAP_SIZE) { func_id = 0; }
  }
}