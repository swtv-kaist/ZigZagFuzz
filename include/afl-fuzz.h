...
struct queue_entry {
  ...
  u32 bitmap_size,                   /* Number of bits set in bitmap     */
      ...
      fuzz_level_argv, n_fuzz_entry; /* offset in n_fuzz                 */
...
enum {
  ...
  /* 21 */ STAGE_ITS,
  /* 22 */ STAGE_HAVOC_ARGV,
  /* 23 */ STAGE_SPLICE_ARGV,
  /* 24 */ STAGE_ARGV_COMB,
  /* 25 */ STAGE_ARGV_FILE_CROSS,
  /* 26 */ FORCE_SELECT,
...
#define FOREIGN_SYNCS_MAX 32U
  ...
  u32 **argv_to_queue;
  u32  *argv_to_queue_cnt;
  u32  *argv_to_queue_size;
  u64  *argv_num_mut;
  u32  *argv_num_finds;
  u64  *argv_num_file_mut;
  u32  *argv_num_file_finds;

  u32 **file_to_queue;
  u32  *file_to_queue_cnt;
  u32  *file_to_queue_size;
  u32   file_to_queue_size_size;
  u32   num_unique_files;
  u64  *file_num_mut;
  u32  *file_num_finds;
  ...
  u32 last_checked_argv;

  u64 interleaving_timeout[2];
  u32 interleaving_finds[2];
  u32 interleaving_prev_finds;
...
void add_to_queue(afl_state_t *, u8 *, u32, u8, u8 *, u32, u8 *);
...
void write_argv_file(afl_state_t *, u8 *, u32);
u32  write_to_testcase(afl_state_t *, void **, u32, u32);
u8   calibrate_case(afl_state_t *, struct queue_entry *, u8 *, u32, u8);
u8   trim_case(afl_state_t *, struct queue_entry *, u8 *);
u8   common_fuzz_stuff(afl_state_t *, u8 *, u32, u8 *, u32);
fsrv_run_result_t fuzz_run_target(afl_state_t *, afl_forkserver_t *fsrv, u32);

void shrink_corpus(afl_state_t *);
void shrink_corpus2(afl_state_t *);
void shrink3(afl_state_t *);
void get_func_rel(afl_state_t *);

void write_mut_log(afl_state_t *);
...
u8 fuzz_one_comb_argv(afl_state_t *);
u8 fuzz_one_concat_comb(afl_state_t *);
u8 fuzz_one_concat_argv(afl_state_t *);
...
void read_old_argv_dir(afl_state_t *);

u32 select_next_queue_entry_argv(afl_state_t *);

void argv_file_crossover(afl_state_t *);
...
void   read_testcases(afl_state_t *, u8 *, char **);
...
void   setup_fsrv_out_argv(afl_state_t *);
...
void log_queue_entry(afl_state_t *);
